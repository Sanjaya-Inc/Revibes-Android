package com.carissa.revibes.core.presentation.compose.components.textfield.base
...
import com.carissa.revibes.core.presentation.compose.foundation.ProvideContentColorTextStyle
...
internal fun CommonDecorationBox(
    ...
    TextFieldTransitionScope.Transition(
        ...
        val decoratedLabel: @Composable (() -> Unit)? =
            label?.let {
                @Composable {
                    Decoration(labelColor, RevibesTheme.typography.label1, it)
                }
            }
        ...
        val decoratedPlaceholder: @Composable ((Modifier) -> Unit)? =
            if (placeholder != null && transformedText.isEmpty() && placeholderAlphaProgress > 0f) {
                @Composable { mod ->
                    Box(mod.alpha(placeholderAlphaProgress)) {
                        Decoration(
                            contentColor = placeholderColor,
                            typography = RevibesTheme.typography.input,
                            content = placeholder,
                        )
                    }
                }
            ...
            supportingText?.let {
                @Composable {
                    Decoration(
                        ...
                        typography = RevibesTheme.typography.body2,
                        content = it
                    )
                }
            }
